    ...
    <div className="min-h-screen transition-colors duration-300" style={{ background: 'var(--bg-surface)' }}>
      ...
      <section className="relative overflow-hidden pt-16 pb-24">
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute top-20 right-[-200px] w-[600px] h-[600px] rounded-full border dark:border-white/5 border-dark/5" />
          <div className="absolute top-40 right-[-100px] w-[400px] h-[400px] rounded-full border dark:border-white/5 border-dark/5" />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="animate-slide-up">
              ...
              <h1 className="font-display text-5xl sm:text-6xl lg:text-7xl font-bold dark:text-white text-dark leading-[1.05] mb-6">
                Never Lose<br />
              ...
              <div className="flex gap-8 flex-wrap">
                {STATS.map(stat => (
                  <div key={stat.label}>
                    ...
                    <div className="text-xs dark:text-white/40 text-dark/50 mt-0.5 font-medium">{stat.label}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="animate-fade-in relative">
              <div className="absolute -top-6 -left-6 w-12 h-12 text-lime animate-spin-slow flex items-center justify-center">
                <svg viewBox="0 0 24 24" fill="#B9FF66" className="w-10 h-10"><path d="M12 2l2.4 7.4H22l-6.2 4.5 2.4 7.4L12 17l-6.2 4.3 2.4-7.4L2 9.4h7.6z"/></svg>
              </div>
              ...
              <div className="absolute -bottom-4 left-6 bg-lime text-dark font-bold px-5 py-3 rounded-2xl shadow-xl flex items-center gap-2 text-sm">
                <CheckCircle size={18} /> No App Required for Finder
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ── Marquee ── */}
      <section className="py-10 border-y overflow-hidden transition-colors" style={{ borderColor: 'var(--border-color)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 mb-6 text-center">
          <p className="dark:text-white/30 text-dark/40 text-sm font-medium uppercase tracking-widest">Works with everything you own</p>
        </div>
        <div className="relative flex overflow-hidden">
          <div className="marquee-track flex gap-12 items-center whitespace-nowrap px-8">
            {['🔑 Keys', '💻 Laptop', '📱 Phone', '🎒 Backpack', '👜 Wallet', '📷 Camera', '🎸 Instruments', '🔑 Keys', '💻 Laptop', '📱 Phone', '🎒 Backpack', '👜 Wallet', '📷 Camera', '🎸 Instruments'].map((item, i) => (
              <span key={i} className="dark:text-white/40 text-dark/50 font-semibold text-base flex-shrink-0 hover:text-lime transition-colors">{item}</span>
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-14">
            <div>
              ...
              <h2 className="font-display text-4xl sm:text-5xl font-bold dark:text-white text-dark max-w-md">Three Simple Steps to Safety</h2>
            </div>
            <p className="dark:text-white/50 text-dark/50 max-w-xs text-sm leading-relaxed md:text-right">From setup to recovery in minutes. No complicated process, no app downloads needed.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {STEPS.map((s) => {
              const isLime = s.variant === 'lime'
              return (
                <div key={s.step} className={`rounded-3xl p-8 border-2 transition-all duration-300 hover:-translate-y-2 card-glow group ${isLime ? 'bg-lime border-lime' : ''}`}
                  style={!isLime ? { background: 'var(--bg-card)', borderColor: 'var(--border-color)' } : {}}>
                  <div className={`text-6xl font-bold mb-6 ${isLime ? 'text-dark/30' : 'dark:text-white/10 text-dark/10'}`}>{s.step}</div>
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-5 ${isLime ? 'bg-dark/10' : 'bg-lime/10'}`}>
                    <div className={isLime ? 'text-dark' : 'text-lime'}>{s.icon}</div>
                  </div>
                  <h3 className={`font-display text-xl font-bold mb-3 ${isLime ? 'text-dark' : 'dark:text-white text-dark'}`}>{s.title}</h3>
                  <p className={`text-sm leading-relaxed ${isLime ? 'text-dark/70' : 'dark:text-white/50 text-dark/60'}`}>{s.desc}</p>
                  <div className={`mt-6 flex items-center gap-2 text-sm font-semibold ${isLime ? 'text-dark' : 'text-lime'}`}>
      ...
      {/* ── Features ── */}
      <section className="py-24 transition-colors" style={{ background: 'var(--bg-card)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-14">
            <div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {FEATURES.map((f) => {
              ...
              return (
                <div key={f.title} className={`rounded-3xl p-7 border-2 transition-all duration-300 hover:-translate-y-2 card-glow ${isLime ? 'bg-lime border-lime' : ''}`}
                  style={!isLime ? { background: 'var(--bg-surface)', borderColor: 'var(--border-color)' } : {}}>
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-5 ${isLime ? 'bg-dark text-lime' : 'bg-lime/10 text-lime'}`}>{f.icon}</div>
                  <h3 className={`font-bold text-lg mb-2 ${isLime ? 'text-dark' : 'dark:text-white text-dark'}`}>{f.title}</h3>
                  <p className={`text-sm leading-relaxed ${isLime ? 'text-dark/70' : 'dark:text-white/50 text-dark/60'}`}>{f.desc}</p>
                </div>
              )
            })}
          </div>
      ...
      <section className="py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="mb-14">
            ...
            <h2 className="font-display text-4xl sm:text-5xl font-bold dark:text-white text-dark max-w-md">What Our Users Say</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {TESTIMONIALS.map((t, i) => (
              <div key={t.name} className={`rounded-3xl p-8 border-2 transition-all duration-300 hover:-translate-y-1 card-glow ${i === 1 ? 'bg-lime border-lime' : ''}`}
                style={i !== 1 ? { background: 'var(--bg-card)', borderColor: 'var(--border-color)' } : {}}>
                <StarRating count={t.rating} />
                <p className={`text-sm leading-relaxed mb-6 ${i === 1 ? 'text-dark/80' : 'dark:text-white/60 text-dark/60'}`}>"{t.text}"</p>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${i === 1 ? 'bg-dark text-lime' : 'bg-lime text-dark'}`}>{t.name.charAt(0)}</div>
                  <div>
                    <div className={`font-bold text-sm ${i === 1 ? 'text-dark' : 'dark:text-white text-dark'}`}>{t.name}</div>
                    <div className={`text-xs ${i === 1 ? 'text-dark/60' : 'dark:text-white/40 text-dark/50'}`}>{t.role}</div>
                  </div>
                </div>
              </div>
            ))}
              ...
              {[
                ...
                <details key={i} className="group rounded-2xl overflow-hidden border transition-colors" style={{ background: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
                  <summary className="flex items-center justify-between p-6 cursor-pointer dark:text-white text-dark font-semibold hover:text-lime transition-colors list-none">
                    {faq.q}
                    <span className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ml-4 group-open:bg-lime group-open:text-dark transition-all" style={{ background: 'var(--border-color)' }}>
                      <span className="text-lg font-light group-open:hidden">+</span>
                      <span className="text-lg font-light hidden group-open:block">−</span>
                    </span>
                  </summary>
                  <div className="px-6 pb-6 dark:text-white/50 text-dark/60 text-sm leading-relaxed">{faq.a}</div>
                </details>
              ))}
            </div>
          </div>
      ...
      {/* ── Contact bottom ── */}
      <section className="py-20 border-t transition-colors" style={{ borderColor: 'var(--border-color)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="rounded-4xl p-12 text-center border transition-colors" style={{ background: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
      ...
      {/* Footer */}
      <footer className="border-t py-12 transition-colors" style={{ background: 'var(--bg-surface)', borderColor: 'var(--border-color)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
